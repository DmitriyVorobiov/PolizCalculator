...
public class Performer {
	...
	private static double calcUnOp(double x, String unOp) {
		switch (unOp) {
		case "sin":
			return Math.sin(x);
		case "cos":
	...
	private static double calcBinOp(double x,double y, char op) {
		switch (op) {
		case '+':
			return x+y;
		case '-':
	...
	static Stack<String> divide(String in) throws InputException {
		...
		for (int i = 0; i < in.length();) {
			if (Character.isDigit(in.charAt(i))) {
				...
				i++;
			} else if (isBinOp(in.charAt(i))) {
				out.push(in.substring(i, i + 1));
				i++;
			} else if (isBkt(in.charAt(i))) {
				out.push(in.substring(i, i + 1));
				i++;
			} else if (isUnOp(in.substring(i, i + 3))) {
				out.push(in.substring(i, i + 3));
				i += 3;
			} else
				throw new InputException("incorrect input", i);
	...
	protected Stack<String> split(String in) throws InputException {
		...
		while (!inputString.empty()) {
			...
			} else if (isUnOp(inputString.peek())) {
				outputString.push(inputString.pop());
			} else if (isBinOp(inputString.peek().charAt(0))) {
				if (store.isEmpty()) {
			...
			} else if (isBkt(inputString.peek().charAt(0))) {
	...
	public double interpretation(String in) throws InputException {
		Stack<String> poliz = reverse(split(in));
		...
		while (!poliz.empty()) {
			if (Character.isDigit(poliz.peek().charAt(0))) {
				...
				Double dig = out.pop();
				out.push(calcUnOp(dig, poliz.pop()));
			} else if (isBinOp(poliz.peek().charAt(0))) {
				// TODO NEG
				char operator = poliz.pop().charAt(0);
				Double dig = out.pop();
				Double dig2 = out.pop();
				out.push(calcBinOp(dig2, dig, operator));
			}
		}
		return out.pop();
	}

	private Stack<String> reverse(Stack<String> split) {
